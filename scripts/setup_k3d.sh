...
# TODO: https://github.com/nolar/setup-k3d-k3s/issues/24
# Then replace action with common one after it's fixed
# but also... not terrible to be able to run this locally??
# See the registry/cluster create commands below for modification in addition to defaults here
...
if [[ -z "${SKIP_CREATION}" ]]; then
  if [[ -z "${SKIP_REGISTRY_CREATION}" ]]; then
    echo "Creating a local registry."
    k3d registry create ${REGISTRY_NAME} --port ${REGISTRY_PORT}
    k3d cluster create ${K3D_NAME:-} --wait --image=rancher/k3s:"${K3S//+/-}" ${K3D_ARGS:-} --registry-use k3d-${REGISTRY_NAME}:${REGISTRY_PORT}
    # Communicate back to GitHub Actions.
